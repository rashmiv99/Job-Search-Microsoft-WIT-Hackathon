    ...
    return(
        <Box p={2} className={classes.wrapper}>
            <Grid container alignItems="center">
                ...
                <Grid item container direction="column" alignItem="flex-end" xs>
                    <Grid item xs>
                        <Typography>{`${props.salary}`}</Typography>
                        <Typography variant="caption">{`${props.type}`} | {differenceInDays(Date.now(), props.postedOn)} days ago | {`${props.location}`}</Typography>
                    </Grid>
                    <Box mt={2}>
